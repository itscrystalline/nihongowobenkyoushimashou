...
def GetDataByID(id : int,page : int = 1,perpage : int = 1, token : str = "") -> dict:
    """
    ...
    if token != "":
        dr.get(re_url+"&pagingToken="+token)
    else:
        dr.get(re_url)
    bs = BeautifulSoup(dr.page_source,features="html.parser").find("pre")
    return json.loads(bs.contents[0])
    ...
    def GetUrlTextToSpeech(self,side : int, speed : int | float | str = "Normal") -> str:
        ...
        match side:
            case 1:
                ...
                # 2nd return
                try:
                    return "https://quizlet.com" + self.side1[0]["ttsurl"][:self.side1[0]["ttsurl"].rfind("&speed")] + "&speed="+ str(speed)
                except TypeError:
                    return None
            case 2:
                try:
                    return "https://quizlet.com" + self.side2[0]["ttsurl"][:self.side2[0]["ttsurl"].rfind("&speed")] + "&speed=" + str(speed)
                except TypeError:
                    return None
            case _:
                raise ValueError("side must be 1 or 2")
